...
public class RpcError {
    ...
    public static class RpcErrorInfo {
        ...
        String BAD_ATTRIBUTE = "bad-attribute";
        String BAD_ELEMENT = "bad-element";
        String BAD_NAMESPACE = "bad-namespace";
        String SESSION_ID = "session-id";
        String OK_ELEMENT = "ok-element";
        String ERR_ELEMENT = "err-element";
        String NOOP_ELEMENT = "noop-element";

        public String getType() {
            if (!Strings.isNullOrEmpty(badAttribute)) {
                return BAD_ATTRIBUTE;
            } else if (!Strings.isNullOrEmpty(badElement)) {
                return BAD_ELEMENT;
            } else if (!Strings.isNullOrEmpty(badNamespace)) {
                return BAD_NAMESPACE;
            } else if (!Strings.isNullOrEmpty(sessionId)) {
                return SESSION_ID;
            } else if (!Strings.isNullOrEmpty(okElement)) {
                return OK_ELEMENT;
            } else if (!Strings.isNullOrEmpty(errElement)) {
                return ERR_ELEMENT;
            } else if (!Strings.isNullOrEmpty(noOpElement)) {
                return NOOP_ELEMENT;
            }
