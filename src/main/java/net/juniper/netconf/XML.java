...
public class XML {
    ...
    public List<Node> findNodes(List<String> list) {
        ...
        // We will iterate through the matching node name list.
        for (int k = 0; k < list.size(); k++) {
            // Set to true when we have a matching node name.
            nextElementFound = false;

            // Get the next name to match.
            String nextElementName = list.get(k);

            // Is this not an element type/value pair match criteria.
            if (!nextElementName.contains("~")) {
                try {
                    // Get the node matching the next element name we are looking for.
                    NodeList nextElementList = nextElement.getElementsByTagName(nextElementName);

                    /* If the next to next(n2n) element is a filter based on text value,
                     ...
                     *         <logical-interface>
                     *             ....
                     * In this case, the list passed to findValue function should contain
                     * (..,"physical-interface","name~ge-1/0/0", "logical-interface",..)
                     * This will fetch me the required element.
                     */

                    // Get the next match criteria.
                    String n2nString = null;
                    if (k < list.size() - 1) {
                        n2nString = list.get(k + 1);
                    }
                    ...
                    if (n2nString != null && n2nString.contains("~")) {
                        /* Since the n2n element is a filter based on text value ( decided by '~')
                         * we now traverse the entire NodeList to find the correct nextElement
                         * based on the text value of the filter.
                         */
                        ...
                        for (int i = 0; i < nextElementList.getLength(); i++) {
                            nextElement = (Element)nextElementList.item(i);
                            Element n2nElement = (Element)nextElement.getElementsByTagName(n2nElementName)
                                .item(0);
    ...
    private static String XML_FORMAT_XSL =
    ...
    public static String getNodeString(Node node, int indent, boolean ignoreDeclaration) {
        try {
            StringWriter writer = new StringWriter();
            TransformerFactory transformerFactory = TransformerFactory.newInstance();
            transformerFactory.setAttribute("indent-number", indent);
            Transformer transformer = transformerFactory.newTransformer(new StreamSource(new StringReader(XML_FORMAT_XSL)));
